...
public class TemplateUtil {
	...
	public String template(TableBean tableBean){
		...
		template = StringUtil.replaceAll(template, "{database.table.name.hump.lower}", HumpUtil.lower(tableBean.getName()));
		
		//{codeblock.field}
		if(template.indexOf("{codeblock.field}") > -1) {
		...
		// {codeblock.method}
		if(template.indexOf("{codeblock.method}") > -1) {
		...
		/**** tostring ****/
		if(template.indexOf("{java.tostring}") > -1) {
